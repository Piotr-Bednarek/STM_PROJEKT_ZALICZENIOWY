...
#include <stdint.h>
#include "stm32f7xx_hal.h" // Changed for STM32F7
#include "string.h"
#include <stdio.h>
#include "VL53L0X.h"
...
uint16_t g_ioTimeout = 0;  // Timeout wyłączony - sensor ma własny measurement timing budget
...
void writeReg(uint8_t reg, uint8_t value) {
  ...
  i2cMsgBuffer[0] = value; // Assign the value to the buffer.
  i2cStat = HAL_I2C_Mem_Write(&VL53L0X_I2C_Handler, g_i2cAddr | I2C_WRITE, reg, 1, i2cMsgBuffer, 1, I2C_TIMEOUT);
  if (i2cStat != HAL_OK) {
      char msg[32];
      sprintf(msg, "I2C Wr Err: %d Rg: %02X\r\n", i2cStat, reg);
...
void writeReg16Bit(uint8_t reg, uint16_t value){
  ...
  memcpy(i2cMsgBuffer, &value, 2); // Assign the value to the buffer.
  i2cStat = HAL_I2C_Mem_Write(&VL53L0X_I2C_Handler, g_i2cAddr | I2C_WRITE, reg, 1, i2cMsgBuffer, 2, I2C_TIMEOUT);
  if (i2cStat != HAL_OK) {
      HAL_UART_Transmit(&huart3, (uint8_t*)"I2C Wr16 Err\r\n", 14, 10);
  }
}
...
void writeReg32Bit(uint8_t reg, uint32_t value){
  ...
  memcpy(i2cMsgBuffer, &value, 4); // Assign the value to the buffer.
  i2cStat = HAL_I2C_Mem_Write(&VL53L0X_I2C_Handler, g_i2cAddr | I2C_WRITE, reg, 1, i2cMsgBuffer, 4, I2C_TIMEOUT);
  if (i2cStat != HAL_OK) {
       HAL_UART_Transmit(&huart3, (uint8_t*)"I2C Wr32 Err\r\n", 14, 10);
   }
}
...
uint16_t readReg16Bit(uint8_t reg) {
  ...
  i2cStat = HAL_I2C_Mem_Read(&VL53L0X_I2C_Handler, g_i2cAddr | I2C_READ, reg, 1, i2cMsgBuffer, 2, I2C_TIMEOUT);
  if (i2cStat != HAL_OK) {
      HAL_UART_Transmit(&huart3, (uint8_t*)"I2C Rd16 Err\r\n", 14, 10);
  } else {
      memcpy(&value, i2cMsgBuffer, 2);
  }
...
uint32_t readReg32Bit(uint8_t reg) {
  uint32_t value = 0;
  i2cStat = HAL_I2C_Mem_Read(&VL53L0X_I2C_Handler, g_i2cAddr | I2C_READ, reg, 1, i2cMsgBuffer, 4, I2C_TIMEOUT);
  if (i2cStat != HAL_OK) {
       HAL_UART_Transmit(&huart3, (uint8_t*)"I2C Rd32 Err\r\n", 14, 10);
   } else {
       memcpy(&value, i2cMsgBuffer, 4);
   }
...
void writeMulti(uint8_t reg, uint8_t const *src, uint8_t count){
  ...
  memcpy(i2cMsgBuffer, src, 4);
  i2cStat = HAL_I2C_Mem_Write(&VL53L0X_I2C_Handler, g_i2cAddr | I2C_WRITE, reg, 1, i2cMsgBuffer, count, I2C_TIMEOUT);
}
...
bool initVL53L0X(bool io_2v8, I2C_HandleTypeDef *handler){
  ...
  if (modelId != 0xEE) {
      char msg[64];
      sprintf(msg, "Invalid Model ID: 0x%02X (Expected 0xEE)\r\n", modelId);
      HAL_UART_Transmit(&huart3, (uint8_t*)msg, strlen(msg), 100);
      // Optional: return false if strict checking is desired, but let's try reset
  } else {
	  HAL_UART_Transmit(&huart3, (uint8_t*)"Model ID OK: 0xEE\r\n", 19, 100);
  }
  ...
  // Set default timeout if not set, to avoid infinite loops during init
  if (g_ioTimeout == 0) {
    g_ioTimeout = 1000; // 1000ms
  }
  ...
  if (!getSpadInfo(&spad_count, &spad_type_is_aperture)) { 
      HAL_UART_Transmit(&huart3, (uint8_t*)"Init Fail: getSpadInfo\r\n", 24, 100);
      return false; 
  }
  ...
  if (!performSingleRefCalibration(0x40)) { 
      HAL_UART_Transmit(&huart3, (uint8_t*)"Init Fail: VHV Calib\r\n", 22, 100);
      return false; 
  }
  ...
  if (!performSingleRefCalibration(0x00)) { 
      HAL_UART_Transmit(&huart3, (uint8_t*)"Init Fail: Phase Calib\r\n", 24, 100);
      return false; 
  }
