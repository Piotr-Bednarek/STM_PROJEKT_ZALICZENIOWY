...
ok. $2{,}2T$. Jeśli przyjąć, że dla małych skoków dynamika jest zbliżona do czasu 
...
Rozpatrując dynamikę piłeczki na belce, wyprowadza się równanie wiążące kąt belki 
...
Obserwując siły działające wzdłuż belki:
\begin{itemize}
...
Z warunku toczenia bez poślizgu, podstawiając $\ddot{\alpha}$ do równania momentu:
\begin{equation}
T \cdot R = J \cdot \frac{\ddot{x}}{R} \quad \Rightarrow \quad T = \frac{J \cdot \ddot{x}}{R^2}
\end{equation}

Wstawiając to wyrażenie na $T$ do równania ruchu postępowego:
\begin{equation}
...
Dla małych kątów $\sin(\theta) \approx \theta$, ostatecznie otrzymuje się:
\begin{equation}
...
Stosując transformatę Laplace'a. Dla warunków początkowych ($x(0) = 0$, $\dot{x}(0) = 0$, $\theta(0) = 0$):
\begin{equation}
...
Posiadając kompletny zestaw opisu dynamik elementów układu, można teraz połączyć 
...
Stąd wynika:
\begin{equation}
X(s) = k_{\mathrm{mech}} \cdot G_{\mathrm{ball}}(s) \cdot \Theta_{\mathrm{serwo}}(s)
\end{equation}

Teraz, znając że $\Theta_{\mathrm{serwo}}(s) = G_{\mathrm{serwo}}(s) \cdot U(s)$, otrzymuje się:
