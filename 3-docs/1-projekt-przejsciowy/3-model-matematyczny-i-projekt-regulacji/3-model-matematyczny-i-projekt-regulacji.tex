...
\begin{equation}
    \sigma^2 = \frac{1}{n-1}\sum_{i=1}^{n}\bigl(K_i - \bar{K}\bigr)^2
        = 0{,}1570\;\left[\frac{\mathrm{mm}}{\mathrm{s}^2 \cdot {^\circ}}\right]^2,
    \label{eq:Kvar}
\end{equation}
\begin{equation}
    \sigma  = 0{,}3962\;\frac{\mathrm{mm}}{\mathrm{s}^2 \cdot {^\circ}}.
    \label{eq:Kstd}
\end{equation}
...
Uzyskany w ten sposób wektor wzmocnień $\mathbf{K}$ przeniesiono bezpośrednio do modułu
...
W klasycznym regulatorze PID gdzie sygnał sterujacy ma postać:
\begin{equation}
  u = K_p e(t) + K_i \int e(t)dt + K_d \frac{de(t)}{dt}
\end{equation}
człon różniczkujący wyliczany jest na podstawie
uchybu regulacji. W przypadku skokowej zmiany
...
przez wzmocnienie wywoływałyby drgania serwomechanizmu,
...
\end{equation}
gdzie stała czasowa filtra $T_f = 0{,}03$~s została dobrana tak, aby tłumić
...
60--140 stopni, co odpowiada odchyleniu $\Theta = \pm 40^{\circ}$). W momencie
...
\textit{clamping} (blokowanie całki). Całkowanie uchybu jest wstrzymywane
 w momencie, gdy sygnał generowany przez regulator jest nasycony, a znak uchybu wskazuje
na to, że dalsze całkowanie pogłębiłoby nasycenie. 
