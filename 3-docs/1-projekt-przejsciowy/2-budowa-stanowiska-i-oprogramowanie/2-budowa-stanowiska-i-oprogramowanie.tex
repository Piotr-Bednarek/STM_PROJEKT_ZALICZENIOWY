...
swobodnie toczy się kulka (pileczka ping pongowa) \ref{fig:stanowisko}.
...
Głównym procesorem stanowiska jest mikrokontroler STM32F767ZI~\cite{stm32f767zi} \ref{fig:stm}
...
Do sterowania kątem belki zastosowano serwomechanizm TowerPro MG946R \ref{fig:serwo} które, przyjmuje sygnał PWM o~częstotliwości 50\,Hz i~czasie wypełnienia
...
Stanowisko wyposażone jest w~podsystem wizji komputerowej, którego zadaniem jest
bezdotykowy pomiar położenia kulki oraz kąta nachylenia belki. Zastosowanie kamery
...
z~okresem 1\,ms (\texttt{camera\_timer.start(1)}), dzięki czemu aplikacja próbuje
odebrać i~przetworzyć obraz tak szybko, jak pozwala na to rzeczywista liczba
...
wraca do szerszego obszaru obejmującego środkową część obrazu.

Parametry progowania HSV, rozmiar filtra rozmywającego oraz próg minimalnego pola
konturu mogą być zmieniane z~poziomu GUI. Ustawienia są zapisywane do pliku
...
\begin{itemize}
  ...
  \item rozluźnione ograniczenia geometryczne detektora, w~tym mniejszy minimalny
...
składowej pionowej jest odwracany ze względu na układ współrzędnych obrazu, w~którym
oś $y$ rośnie w~dół. W drugiej metodzie obliczany jest kąt między dwoma wektorami:
